...
function main(params) {
    ...
    let dealerships = getMatchingRecords(cloudant, 'dealerships', 
    { state: params.state, id: params.dealerid ? parseInt(params.dealerid) : undefined });
    return dealerships;
}
 ...
 function getMatchingRecords(cloudant,dbname, selector) {
     return new Promise((resolve, reject) => {
         cloudant.postFind({db:dbname,selector:selector})
                 .then((result)=>{
                   resolve({body:result.result.docs});
                 })
                 .catch(err => {
                    console.log(err);
                     reject({ err: err });
